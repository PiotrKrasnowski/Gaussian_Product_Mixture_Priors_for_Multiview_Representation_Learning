...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        self.occlusionn_coeff = args["occlusion_coeff"]

        if (len(self.degrees_coeff) != self.n_views) or (len(self.translate_coeff) != self.n_views) or (len(self.scale_coeff) != self.n_views) or (len(self.PixelCorruption_coeff) != self.n_views) or (len(self.occlusionn_coeff) != self.n_views):
    ...
    def train_full(self):
        print('Loss ID:{}, Beta joint:{:.0e}, NumCent:{}, MovCoeff:{}, Mode:{}, Loss Choice:{}, update_center_rule:{}'.format(self.loss_id,self.beta_joint,self.M,self.moving_coefficient_multiple,self.model_mode,self.loss_choice,self.update_centers_rule))
    ...
    def train_step1(self):
        ...
        for epoch in range(self.epoch_1):
            ...
            for idx in range(len(y_d)//self.batch_size):
                ...
                if optimization_mode != 'no_centers' and self.loss_id == 2:
